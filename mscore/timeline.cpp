...
void Timeline::setScore(Score* v)
      {
      _cv    = 0;
      _score = v;
      rescale();
...
void Timeline::paintEvent(QPaintEvent* ev)
      ...
      //std::cout << _score->firstMeasure()->timesig().numerator() << std::endl;

//      qDebug("timeline paint %d %d", r.width(), r.height());
